...
## 클레버 검수 (2026-07-31 04:xx · 개발팀 팀장 단독 방)

### 4축 검수 결과
- **정확성**: 수정 (공휴일대체 근거 법률 개정연도 오타 1건 · 아래 수정 항목 참조)
- **완성도**: 수정 (디자인팀 피카소 시안 **미반영** — 두 파일 모두 마이클 초안 그대로 · design-notes 33개 지시 항목 반영 0)
- **원칙**: OK (SEO 태그·JSON-LD·접근성 aria·광고 슬롯·JS 문법 전부 통과 · 체감온도 `node --check` PASS · 공휴일대체 JS 없음)
- **배포준비**: **배포 불가** — 브랜드 승계 파탄 + 체감온도 마이클 원본 `#0ea5e9` 대비 약 2.7:1 (WCAG AA 4.5:1 심각 미달)

### 수정 항목 (클레버 직접 수정)

**public-holiday-substitute-2026/index.html · 대체공휴일 근거 법률 개정연도 오타 정정**
- `관공서의 공휴일에 관한 규정 (2022년 개정)` → `(2023년 개정)`
- 이유: 위 대상 리스트에 부처님오신날·크리스마스가 포함되어 있는데 이 두 항목의 대체공휴일 대상 편입은 2023년 5월 대통령령 개정 시점. 2022년 개정은 어린이날만 추가된 시점이라 리스트와 근거 연도가 논리적으로 어긋남. 법률 참조 문구는 신뢰 앵커라 배포 전 반드시 정정. HTML 주석에 `[클레버 수정: 2022년 → 2023년 개정 · 대상 리스트에 부처님오신날·크리스마스 포함 근거]` 명시.

**추가 검증 완료 (오류 없음)**:
- 광복절 8/15(토) → 대체 8/17(월) · 8월 캘린더 요일 전부 실제 대조 일치 (Python date 검증)
- 연차 조합 표 5개 케이스 (0/1/2/4/5일) 연속 일수 계산 전부 정확
- 캘린더 스트립 8/8~8/17 10셀 요일 라벨 전부 정확
- 하반기 대체공휴일 리스트: 광복절 8/17(월) · 개천절 10/5(월) · 추석 9/28(월) 예상 모두 정확
- 각주 "한글날(10/9 금)·크리스마스(12/25 금)는 평일" 요일 정확
- 대체공휴일 대상 공휴일 열거 (3·1절·광복절·개천절·한글날·어린이날·설날·추석·부처님오신날·크리스마스) 완비
- 체감온도 계산: Rothfusz 열지수 계수·저습·고습 보정·°F 변환 정확 / NWS·캐나다 풍속냉각지수 공식 정확 / SUMMER·WINTER threshold 배열 순서·경계 처리 정확

### 수정 안 하고 남긴 이슈 (팀장 판단 필요 · 이월)

1. **체감온도 겨울 진입 로직 결함 (기능 개선 대상)**
   - 현재: `if (tc <= 10 && vkmh >= 5)` → 겨울 풍속냉각 진입
   - 결함: `tc = -15, vkmh = 3` 케이스에서 조건 미충족 → `formula: 'neutral'` → 결과 "😊 쾌적 — 외출하기 좋은 날씨" 표기
   - 사용자 경험 파탄: 영하 15℃에 "쾌적·외출하기 좋은 날씨"는 안전 신호 반대. 만약 무풍이면 최소한 windChill 계산 스킵하고 `tc` 그대로를 WINTER threshold에 태워야 함
   - 개선안: `if (tc <= 10)` 로 완화하고, `vkmh < 5`면 `windChillC` 대신 `tc` 그대로 반환하되 winter formula/level 태깅
   - 클레버 판단: 큰 로직 변경 (분기 조건·formula 분류 재설계)이라 세션 스코프 넘어감 · 마이클과 왕복 협의 필요 · **다음 사이클 최우선 수정 대상으로 이월**
   - 배포 전 반드시 fix 필요 (신뢰·안전 신호 이슈)

2. **디자인팀 피카소 시안 전면 미반영 (33개 지시 항목 반영 0) — 지난 사이클과 동일 패턴 반복**
   - `design-notes.md § 피카소 시안 결과` = "(대기)" 상태
   - 미반영 항목: Pretendard CDN · CSS custom properties · body bg · hero-line · @keyframes resultReveal · 버튼 hover transform · 색 팔레트 (체감온도 하늘청 · 공휴일대체 코발트) · disclaimer border · fact-box·formula-badge 3단 소프트 토큰 · rule-bullet 다빈치 보완 · 인라인 style 두 곳(line 243·272) → `.footnote` class 교체
   - 특히 대비비 이슈: 체감온도 마이클 `#0ea5e9` on white ≈ 2.7:1 (WCAG AA 4.5:1 심각 미달) → 디자인팀 `#0072A8` 조정본 필수 반영
   - 클레버가 디자인 지시를 직접 코드에 붙이는 건 왕복 사이클 무력화 · 명의 사칭 소지 → 반영 안 함
   - **시스템 이슈 상신**: 두 사이클(2026-07-29·2026-07-31) 연속으로 클레버 검수 시점에 피카소 시안 미반영 상태 · 파이프라인 병목 존재 (마이클 초안 → 디자인 지시만 있고 피카소 실행이 안 됨) → 사마의 일일보고에서 프로세스 진단 요망

3. **인라인 style 잔존 (공휴일대체 · line 243·272)**
   - `<p style="font-size:0.78rem;color:#aaa;margin-top:0.4rem;">` 두 곳
   - 디자인 지시 30번 = `.footnote` class 추출 지시. 미반영
   - 개발 원칙(스타일 인라인 지양) 관점이지만 디자인팀 지시와 겹치므로 클레버 별도 수정 안 함 (피카소 왕복 시 함께 반영)

### 배포 준비 상태

**배포 불가 · 사유: (a) 디자인팀 피카소 시안 미반영 · 브랜드 승계 파탄 + 체감온도 WCAG AA 심각 미달 (b) 체감온도 겨울 진입 로직 결함 · 저온 무풍 케이스 오판**

배포 조건 (재검수 필요):
1. 피카소 → 달리 → 다빈치 왕복 완료 (design-notes.md 각 섹션 채워짐 · 33개 지시 항목 반영)
2. 체감온도 겨울 진입 로직 수정 (마이클 재작업)
3. 클레버 재검수 (4축 전면 재판정)
