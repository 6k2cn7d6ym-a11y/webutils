...
- **4축 검수 결과**:
  - 정확성: **wedding-gift-calculator OK** · **minimum-wage-2027-dday 조건부 (WAGE_2027 미확정)**
  - 완성도: **OK** (두 파일 모두 다빈치 판정 정확 반영)
  - 원칙: **OK** (수정 후)
  - 배포준비: **wedding-gift-calculator 준비 완료** · **minimum-wage-2027-dday 조건부 준비 · 사유 아래**

### 수정 항목 (총 3건)

**minimum-wage-2027-dday/index.html — 2건**
1. **팩트 표기 안전화 (L269 주변 · h2 + note 문구)** — `WAGE_2027 = 10030` (2026년 값 잠정 대입) 상태에서 UI가 "**최저임금위원회 확정**" · "**2026년 최저임금(10,030원) 대비 변동분**" 표기 → 사용자가 확정값으로 오해할 심각한 리스크. 마케팅팀 팩트 조사 완료 전 안전 문구로 임시 변경:
   - h2: "2027 최저임금 (최저임금위원회 확정)" → "2027 최저임금 (발표 대기 · 2026년 기준 잠정 표시)"
   - note: "2026년 최저임금(10,030원) 대비 변동분은 아래 계산기에서 확인하세요" → "2027년 최저임금은 최저임금위원회 공식 발표 후 자동 반영되며, 현재는 2026년 최저임금(10,030원)을 잠정 기준으로 표시합니다"
   - 원복 지시 주석 인라인 삽입 (마이클이 확정값 반영 시 h2·note·상수를 함께 원복해야 함을 명시)
2. **JS 주석 명확화 (L388 주변)** — 원본 "확인 필요" 한 줄 주석을 "잠정 대입 상태 · 마이클 원복 대상 3곳(상수·h2·note) 명시" 상세 주석으로 교체. WAGE_2026 예비 상수 용도(향후 변동률 계산) 문서화.

**wedding-gift-calculator/index.html — 1건**
3. **preconnect crossorigin 누락 (L18)** — `<link rel="preconnect" href="https://cdn.jsdelivr.net">` → `crossorigin` 추가. Pretendard woff2 폰트 CORS fetch에 필요 · 시리즈 다른 유틸(minimum-wage 포함)과 일관성 확보. 사소하나 preconnect 실효성 최대화.

### 배포 준비 상태

- **wedding-gift-calculator**: **준비 완료** · 즉시 배포 가능. 로직·색·hero-line·2슬롯 광고·크로스링크·SEO 전수 검증 통과.
- **minimum-wage-2027-dday**: **조건부 준비** · 사용자 오해 문구는 안전화 완료로 현 상태 배포는 기술적으로 가능하나, "2027 최저임금" 유틸 본질상 확정값 배포가 정합. 배포 판단 두 옵션 대표께 상신 필요.

### 검증 실증 (마이클·다빈치 인계 항목 재확인)

- **JS 문법 5건 전수 통과** (`node new Function` 파서) — JSON-LD 스크립트 제외 · 실제 JS 5개 모두 파싱 성공
- **shared/ads.css 존재 확인 ✓** (489 bytes · `.ad-slot`·`.ad-slot-label`·`.ad-slot-bottom`·768px 미디어쿼리 완비)
- **minimum-wage 계산 실증**: WAGE=10030 → 시급 10,030 · 일급(8h) 80,240 · 월급(주40h+주휴8h) 2,091,857 · 주급(주40h+주휴8h) 481,440 · 월급(×4.345) 2,091,857 · 연간 25,102,284
- **D-day 실증** (2026-08-17 04:00 KST 기준): D-137
- **요일 검증**: 2027-01-01 = **금요일** ✓ (`period-bar` "2027. 1. 1(금)" 정확)
- **wedding-gift 매트릭스 8케이스 시뮬레이션 통과**:
  - 친구·보통·참석·일반 → 5만 (5~7)
  - 친구·높음·참석·일반 → 10만 (10~15)
  - 동료·보통·불참·일반 → 5만 (5~7)
  - 상사·보통·참석·사초년 → 7만 (7~10)
  - 친척·높음·참석·여유 → 30만 (30~50)
  - 지인·낮음 → "안 내도 무방" (null 분기)
  - 김영란법 상한: 친구·보통 5만 유지 · 친척·높음·여유 30만 → 5만 강제 (상한 로직 정상)

### 전수 보존 확인

- SEO 메타·OG·Twitter·JSON-LD·canonical 무손실 (WebApplication / LifestyleApplication 스키마)
- adsbygoogle INS 태그·`#ad-slot`·`.ad-slot`·`.ad-slot-bottom` 무손실 (wedding-gift 2슬롯 · 768px 미만 하단 숨김 설계 존중)
- JS 계산 로직 무손실 (WAGE_2027 계산 체인·주휴수당 조건·D-day 갱신·M 매트릭스·STEPS·김영란법·participate/situation·공유 로직)
- DOM ID 전수 (minimum-wage 18개·wedding-gift 14개) 무손실
- 크로스링크 무손실 (minimum-wage 근로 유틸 4개·wedding-gift 조의금 계산기 1개)
- Cloudflare Web Analytics 무손실
- 480px 반응형 미디어 쿼리 무손실

### 팔레트 편입 확인 (다빈치 판정 반영)

| 톤 | 색값 | 도메인 |
|-----|------|--------|
| 그린 | `#198754` | 근로 5중 완성 (근로장려금·자녀장려금·연장수당·주휴수당·**최저임금 신규**) |
| 보라 | `#553C9A` | 성인 의례·예절·격식·경조사 신규 편입 (조의금 계산기 예약) |

### 이월 이슈 (배포 블로킹 아님 · 지속 관찰)

1. **WAGE_2027 확정값 반영** — 마케팅팀 팩트 조사 완료 후 마이클이 상수·h2·note 3곳 함께 원복 필수. 순서: 마케팅 사실 확정 → 마이클 반영 → 클레버 재검수 → 배포
2. **Math.ceil 자정 정밀도** — D-day 계산이 KST 자정 경계에서 1일 오차 가능성. 이월 유지 (배포 블로킹 아님)
3. **`<link as="style">` 문법** — 이월 유지
4. **adsbygoogle inline slot 미삽입** — 실사용 확인상 정상 배포됨 · 이월 유지
5. **wedding-gift favicon `#805AD5` vs primary `#553C9A` 톤 차이** — 시각 우선순위 낮음 · 다빈치 판정 시 미언급 · 이월 신규 관찰 항목

### 대표 판단 요청 (사마의 보고 상신)

**wedding-gift-calculator는 즉시 배포 가능. minimum-wage-2027-dday 배포 판단 두 옵션:**

- **A안**: 오늘 wedding-gift만 배포 → 마케팅팀 팩트 조사 완료 후 minimum-wage 별개 배포 (주 3회 사이클 · 다음 수요일 슬롯)
- **B안**: 두 건 동시 배포 (minimum-wage 잠정 표시 상태 그대로 · 팩트 조사 결과 반영은 hotfix로 별도 배포)

**클레버 권고**: **A안**. "2027 최저임금" 유틸을 잠정값 상태로 배포하는 것은 유틸 본질(확정값 안내)에서 이탈 · 사용자 신뢰 리스크. 팩트 확정 후 배포가 정합.

---

**검수**: 클레버 (개발 팀장) · 2026-08-17
**다음**: 사마의 (일일 보고) · 대표 배포 지시 대기 (A안·B안 판단)
