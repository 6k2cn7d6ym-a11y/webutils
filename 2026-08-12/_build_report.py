#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-12 웹유틸 사이클 보고서 조립 · 사마의 · 04-보고 세션"""

import base64
import shutil
from pathlib import Path

BASE = Path("/Users/jim/projects/webutils/2026-08-12")
OUT_LOCAL = BASE / "04-보고서.html"
OUT_OUTBOX = Path("/Users/jim/projects/webutils/_outbox/webutils_보고서_2026-08-12.html")

def b64_iframe(html_path: Path) -> str:
    raw = html_path.read_bytes()
    b64 = base64.b64encode(raw).decode("ascii")
    return f'<iframe class="preview" src="data:text/html;base64,{b64}" title="{html_path.parent.name} 실물 미리보기" loading="lazy"></iframe>'

suneung_iframe = b64_iframe(BASE / "suneung-dday" / "index.html")
defect_iframe = b64_iframe(BASE / "defect-warranty-dday" / "index.html")
...
<main>
  ...
  <div class="decision-card">
    ...
    <ul>
      ...
      <li><strong>광고 슬롯 형식</strong>: 두 파일 모두 옛 형식 <code>&lt;div id="ad-slot"&gt;광고&lt;/div&gt;</code>로 산출됨 (chuseok-ktx·work-grant 배포 2건과 동일 문제) · 개발팀 정합 자율지시([[개발팀]])가 진행 중이므로 <strong>같은 정합 스크립트에 이번 2건도 포함</strong> 권고 · 대표 승인 시 사마의가 자율지시 스코프 확장 통지</li>
...
OUT_OUTBOX.parent.mkdir(parents=True, exist_ok=True)
shutil.copyfile(OUT_LOCAL, OUT_OUTBOX)

# 크기 확인
local_kb = OUT_LOCAL.stat().st_size / 1024
outbox_kb = OUT_OUTBOX.stat().st_size / 1024
print(f"OK · local={local_kb:.1f}KB · outbox={outbox_kb:.1f}KB")
